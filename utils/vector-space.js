...
// A simple tokenizer function for Vietnamese could be more complex
...
function inverseDocumentFrequency(term, allDocuments) {
	...
	return Math.log(allDocuments.length / (1 + count));
}
...
function calculateTFIDF(document, allDocuments) {
	...
	// Remove stopwords and tokenize the document
	const terms = tokenize(document).filter(term => !stopwords.includes(term));
...
function cosineSimilarity(docVector1, docVector2) {
	const intersection = new Set(
		[...docVector1.keys()].filter(x => docVector2.has(x)),
	);
	...
	for (const term of intersection) {
		dotProduct += docVector1.get(term) * docVector2.get(term);
	}

	for (const term of docVector1.keys()) {
		magnitudeA += Math.pow(docVector1.get(term), 2);
	}

	for (const term of docVector2.keys()) {
		magnitudeB += Math.pow(docVector2.get(term), 2);
	}
